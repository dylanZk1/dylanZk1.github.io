...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | Rudolf")[0];
}
...
function changeBeiAn() {
  ...
  if(window.location.host.toString().indexOf("gorudolf.online")!=-1){
    ...
    BeiAnImg.setAttribute("src","https://resource.drobot.online/blog/SuBeiAn.svg")
    BeiAnImg.setAttribute("data-lazy-src","https://resource.drobot.online/blog/SuBeiAn.svg")
...
______               _           _    __   _           ______   _                 
| ___ \\             | |         | |  / _| ( )          | ___ \\ | |                
| |_/ /  _   _    __| |   ___   | | | |_  |/   ___     | |_/ / | |   ___     __ _ 
|    /  | | | |  / _\` |  / _ \\  | | |  _|     / __|    | ___ \\ | |  / _ \\   / _\` |
| |\\ \\  | |_| | | (_| | | (_) | | | | |       \\__ \\    | |_/ / | | | (_) | | (_| |
\\_| \\_|  \\__,_|  \\__,_|  \\___/  |_| |_|       |___/    \\____/  |_|  \\___/   \\__, |
                                                                             __/ |
                                                                            |___/ 
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Rudolf %c 你正在访问Rudolf的博客",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Rudolf") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('Rudolf的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
function createtime() {
  ...
  var start = new Date("09/05/1977 12:56:00"); // 旅行者1号开始计算的时间
  var dis = Math.trunc(((now - start) / 1000) * 17.062); // 距离=秒数*速度 记住转换毫秒
  var unit = (dis / 149600000).toFixed(6);  // 天文单位
  // 网站诞生时间
  var grt = new Date("07/01/2023 00:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='//sourcebucket.s3.bitiful.net/badge/F小屋-科研摸鱼中.svg' title='什么时候能够实现财富自由呀~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='//sourcebucket.s3.bitiful.net/badge/F小屋-下班休息啦.svg' title='下班了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
if (localStorage.getItem("font") == undefined) {
  localStorage.setItem("font", "pudding");
}
setFont(localStorage.getItem("font"));
function setFont(n) {
  localStorage.setItem("font", n)
  if (n == "default") {
    document.documentElement.style.setProperty('--global-font', 'pudding');
    document.body.style.fontFamily = "-apple-system, 'pudding', Consolas_1, BlinkMacSystemFont, 'Segoe UI' , 'Helvetica Neue' , Lato, Roboto, 'PingFang SC' , 'Microsoft JhengHei' , 'Microsoft YaHei' , sans-serif";
  }
  else {
    document.documentElement.style.setProperty('--global-font', n);
    document.body.style.fontFamily = "var(--global-font),'pudding', -apple-system, IBM Plex Mono ,monosapce,'微软雅黑', sans-serif";
  }
